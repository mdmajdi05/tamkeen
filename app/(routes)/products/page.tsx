...
export const metadata: Metadata = {
  title: 'Products',
  description:
    'Explore TAMKEEN product range — power generation equipment, marine automation systems, solar infrastructure, oil & gas components, and turbine spares.',
  alternates: { canonical: 'https://tamkeen-es.com/products/' },
  openGraph: {
    title: 'Products | TAMKEEN',
    description: 'Industrial energy products — power generation, marine automation, solar, oil & gas, and turbine spares.',
    url: 'https://tamkeen-es.com/products/',
    images: [{ url: '/tamkeen-logo.jpeg', width: 1200, height: 630, alt: 'TAMKEEN Products' }],
  },
  twitter: {
    title: 'Products | TAMKEEN',
    description: 'Industrial energy products — power generation, marine automation, solar, oil & gas, and turbine spares.',
...
function ProductCard({ product }: { product: ProductCategory }) {
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
      <div
      ...
      <div className="p-6">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-brand-blue dark:group-hover:text-brand-green transition-colors">
          {product.title}
        </h3>
        ...
        <ul className="mt-4 space-y-1.5">
          {product.features.slice(0, 3).map((feat) => (
            <li key={feat} className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
              <span
        ...
        {product.features.length > 3 && (
          <p className="mt-2 text-xs font-medium" style={{ color: '#1565C0' }}>
...
function ProductsContent({ categories }: { categories: ProductCategory[] }) {
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-20">
        ...
        <div className="relative mx-auto max-w-4xl px-6 text-center lg:px-8">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-gray-500 dark:text-gray-400">
            Premium industrial equipment sourced globally — from turbines and switchgear to solar PV and EV charging infrastructure.
          </p>
        </div>
      </section>
      <section className="pb-24 bg-white dark:bg-gray-950">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {categories.map((cat) => (
              <ProductCard key={cat.id} product={cat} />
            ))}
          </div>
        </div>
      </section>
      <section className="py-16 bg-gray-50 dark:bg-gray-900">
        <div className="mx-auto max-w-3xl px-6 text-center lg:px-8">
          ...
          <p className="mt-4 text-base leading-relaxed text-gray-500 dark:text-gray-400">
            Our global sourcing network can procure specialized equipment tailored to your project specifications.
          </p>
          <Link
            href="/contact"
      ...
      <div className="relative z-10 backdrop-blur-xl bg-slate-900/40 border border-slate-800 p-12 rounded-3xl shadow-2xl max-w-xl w-full flex flex-col items-center border-t-blue-500/30">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-slate-400 max-w-sm mx-auto leading-relaxed">
            Our product catalogue is being updated. Check back shortly.
          </p>
...
export default function ProductsPage() {
  try {
    const categories = (content as any).productCategories;
